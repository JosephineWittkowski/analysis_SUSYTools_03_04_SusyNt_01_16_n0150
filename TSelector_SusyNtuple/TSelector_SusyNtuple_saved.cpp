...
//   if(!(m_baseElectrons.size()==2 || m_baseMuons.size()==2 || (m_baseElectrons.size()+m_baseMuons.size())==2)) return false; //only count leptons where no Mll < 20 GeV. Mll < 12 GeV veto ALREADY DONE FOR SELECTING BASELINE LEPTONS in performOverlapRemoval()
  ...
  float passedTopTag = false; //remove
  ...
  if(m_baseElectrons.size()==2){
    cutnumber = 15.; fillHistos_EE(cutnumber, weight_ALL_EE); //pass category
    if(m_baseElectrons.size()==2 && m_signalElectrons.size()==2 ){
      cutnumber = 16.; fillHistos_EE(cutnumber, weight_ALL_EE); //pass nlep
      Electron* el1;
      Electron* el2;
	  ...
	  cutnumber = 17.; fillHistos_EE(cutnumber, weight_ALL_EE);
	  if(m_trigObjWithoutRU->passDilEvtTrig(leptons, 0., nt.evt())){ //valid pT region
	    cutnumber = 18.; fillHistos_EE(cutnumber, weight_ALL_EE);
	    ...
	    if(nt.evt()->isMC || (!nt.evt()->isMC && m_trigObjWithoutRU->passDilTrigMatch(leptons, 0., nt.evt()))){  //match to trigger (in MC not needed bc weighted)
	      cutnumber = 19.; fillHistos_EE(cutnumber, weight_ALL_EE);
	      if(!nt.evt()->isMC || CheckRealLeptons(electrons, m_signalMuons)){
		cutnumber = 20.; fillHistos_EE(cutnumber, weight_ALL_EE);
		...
		calc_EE_variables(el1, el2, el1_SS_TLV, el2_SS_TLV, met_SS_TLV, signalJet0_TLV, signalJet1_TLV, useForwardJets);
		
		mZTT_coll = calcMZTauTau_coll(el1_SS_TLV, el2_SS_TLV, met_SS_TLV);
		mZTT_mmc = calcMZTauTau_mmc(el1_SS_TLV, el2_SS_TLV, 0/*electron*/, 0/*electron*/);
    // 	    float mZTT_missMassCalc = calcMZTauTau_MissMassCalc(el1_TLV, el2_TLV, 0/*electron*/, 0/*electron*/);
      // 	  cout << "EE mZTT_coll= " << mZTT_coll << " mZTT_mmc= " << mZTT_mmc << " mZTT_missMassCalc= " << mZTT_missMassCalc << endl;
		...
		float mtWW_EE = calcMt((el1_SS_TLV + el2_SS_TLV), met_SS_TLV); //!!! mtWWee
		
		float DeltaPhiLLJJ = (nSignalJets>1) ? (el1_SS_TLV + el2_SS_TLV).DeltaPhi(signalJet0_TLV + signalJet1_TLV) : 0.;
		
		float DeltaPhiMETee = fabs((el1_SS_TLV + el2_SS_TLV).DeltaPhi(met_SS_TLV));
		float mTemin = (calcMt(el1_SS_TLV, met_SS_TLV) > calcMt(el2_SS_TLV, met_SS_TLV)) ? calcMt(el2_SS_TLV, met_SS_TLV) : calcMt(el1_SS_TLV, met_SS_TLV);
		float sum_mv1 = calcSumMv1(m_signalJets2Lep);
		float DeltaRee = fabs(el1_SS_TLV.DeltaR(el2_SS_TLV));
// 		float HT_EE = calcHT(el1_SS_TLV, el2_SS_TLV, met_SS_TLV, m_signalJets2Lep); //!!!! -> HTee
		
		//------------------------------------------------------------------------------------
		//----------------------------------SR-SS-EE------------------------------------------
		//------------------------------------------------------------------------------------
		cutnumber = 21.; fillHistos_EE_SRSS1(mcid, cutnumber, weight_ALL_SS_EE);  //SS cut: applied only on weighted events
		cutnumber = 22.; fillHistos_EE_SRSS1(mcid, cutnumber, weight_ALL_SS_EE); 
		if(fabs(el1->d0Sig(true))<=3.0 && fabs(el2->d0Sig(true))<=3.0){//|d0/sd0|<3   (for electron) (update to latest QFlip)
		  cutnumber = 23.; fillHistos_EE_SRSS1(mcid, cutnumber, weight_ALL_SS_EE); 
		  if(numberOfFJets(m_signalJets2Lep) == 0){
		    weight_ALL_SS_EE *= getBTagWeight(nt.evt());
		    cutnumber = 24.; fillHistos_EE_SRSS1(mcid, cutnumber, weight_ALL_SS_EE); 
		    if(numberOfCBJets(m_signalJets2Lep) == 0){
		      cutnumber = 25.; fillHistos_EE_SRSS1(mcid, cutnumber, weight_ALL_SS_EE); 
		      if(nSignalJets >=1){
			cutnumber = 26.; fillHistos_EE_SRSS1(mcid, cutnumber, weight_ALL_SS_EE); 
			if(el1_SS_TLV.Pt() >= 30. && el2_SS_TLV.Pt()>= 20.){
			  cutnumber = 27.; fillHistos_EE_SRSS1(mcid, cutnumber, weight_ALL_SS_EE); 
			  if((el1_SS_TLV + el2_SS_TLV).M() > MZ+10. || (el1_SS_TLV + el2_SS_TLV).M() < MZ-10.){
			    cutnumber = 28.; fillHistos_EE_SRSS1(mcid, cutnumber, weight_ALL_SS_EE);  //ZVeto
			    
			    //SRSS1
			    if(mtWW_EE >= 150.){
			      
			      cutnumber = 29.; fillHistos_EE_SRSS1(mcid, cutnumber, weight_ALL_EE); 
			      
			      if(HT_EE >= 200.){
				cutnumber = 30.; fillHistos_EE_SRSS1(mcid, cutnumber, weight_ALL_SS_EE); 
				float METrel_SS = recalcMetRel(met_SS_TLV, el1_SS_TLV, el2_SS_TLV, m_signalJets2Lep, useForwardJets);
				if(METrel_SS>=50.){
				  cutnumber = 31.; fillHistos_EE_SRSS1(mcid, cutnumber, weight_ALL_SS_EE); 
				  //SRSS2
				  float mt2 = calcMT2(met_SS_TLV, el1_SS_TLV, el2_SS_TLV);
				  if(mt2>=90.){
				    cutnumber = 32.; fillHistos_EE_SRSS2(mcid, cutnumber, weight_ALL_SS_EE); 
		...
		//------------------------------------------------------------------------------------
		if((el1->q * el2->q)<0){
		  cutnumber = 50.; fillHistos_EE_SROS1(mcid, cutnumber, weight_ALL_EE); 
		  cutnumber = 51.; fillHistos_EE_SROS1(mcid, cutnumber, weight_ALL_EE); 
		  if(numberOfFJets(m_signalJets2Lep) == 0){
		    weight_ALL_EE *= getBTagWeight(nt.evt());
		    cutnumber = 52.; fillHistos_EE_SROS1(mcid, cutnumber, weight_ALL_EE); 
		    if(numberOfCBJets(m_signalJets2Lep) == 0){
		      cutnumber = 53.; fillHistos_EE_SROS1(mcid, cutnumber, weight_ALL_EE); 
		      if(nSignalJets >=2){
			cutnumber = 54.; fillHistos_EE_SROS1(mcid, cutnumber, weight_ALL_EE); 
			TLorentzVector signalJet0_TLV, signalJet1_TLV;
			signalJet0_TLV.SetPtEtaPhiE(jet0->pt, jet0->eta, jet0->phi, jet0->pt*cosh(jet0->eta));
			signalJet1_TLV.SetPtEtaPhiE(jet1->pt, jet1->eta, jet1->phi, jet1->pt*cosh(jet1->eta));
			mjj = Mll(m_signalJets2Lep.at(0), m_signalJets2Lep.at(1));
			if(mjj >= 50. && mjj <= 100.){
			  cutnumber = 55.; fillHistos_EE_SROS1(mcid, cutnumber, weight_ALL_EE); 
			  if(el1_TLV.Pt() >= 30 && el2_TLV.Pt() >= 30){
			    cutnumber = 56.; fillHistos_EE_SROS1(mcid, cutnumber, weight_ALL_EE); 
			    float DeltaRee = fabs(el1_TLV.DeltaR(el2_TLV));
			    if(DeltaRee<1.5){
			      cutnumber = 57.; fillHistos_EE_SROS1(mcid, cutnumber, weight_ALL_EE); 
			      
			      if(mTemin >= 60.){
				cutnumber = 58.; fillHistos_EE_SROS1(mcid, cutnumber, weight_ALL_EE); 
				float DeltaPhiMETee = fabs((el1_TLV + el2_TLV).DeltaPhi(m_met->lv()));
				if(DeltaPhiMETee>1.5){
				  cutnumber = 59.; fillHistos_EE_SROS1(mcid, cutnumber, weight_ALL_EE); 
				  if(m_met->lv().Pt() >= 80.){
				    cutnumber = 60.; fillHistos_EE_SROS1(mcid, cutnumber, weight_ALL_EE); 
				  }
				}
			      }
			    }
  ...
  if(m_baseMuons.size()==2){
    cutnumber = 15.; fillHistos_MM(cutnumber, weight_ALL_MM); //pass category
	...
	cutnumber = 16.; fillHistos_MM(cutnumber, weight_ALL_MM); //pass nlep
	...
	if(m_signalTaus.size() == 0){
	  cutnumber = 17.; fillHistos_MM(cutnumber, weight_ALL_MM);
	  if(m_trigObjWithoutRU->passDilEvtTrig(leptons, 0., nt.evt())){ //valid pT region
	    cutnumber = 18.; fillHistos_MM(cutnumber, weight_ALL_MM);
	    if(m_signalTaus.size() == 0){
	      
	      if(nt.evt()->isMC || (!nt.evt()->isMC && m_trigObjWithoutRU->passDilTrigMatch(leptons, 0., nt.evt()))){ //match to trigger
		cutnumber = 19.; fillHistos_MM(cutnumber, weight_ALL_MM);

		if(!nt.evt()->isMC || CheckRealLeptons(m_signalElectrons, muons)){
		  cutnumber = 20.; fillHistos_MM(cutnumber, weight_ALL_MM);
		  //time to calc weights:
		  //------------------------------------------------------------------------------------
		  //calc lepton SF:		  
		  float lep_SF_MM = 1.0;
		  ...
		  //calc trigger weight:
		  float trigW_MM = 1.;
		  if(nt.evt()->isMC) trigW_MM = m_trigObjWithoutRU->getTriggerWeight(leptons, nt.evt()->isMC, m_met->lv().Et(), nSignalJets, nt.evt()->nVtx, NtSys_NOM);
		  //product of all weights:
		  ...
		  met_TLV = m_met->lv();
		  calc_MM_variables(mu1, mu2, mu1_TLV, mu2_TLV, met_TLV, signalJet0_TLV, signalJet1_TLV, useForwardJets);
		  float HT_MM = calcHT(mu1_TLV, mu2_TLV, m_met->lv(), m_signalJets2Lep);
		  float mtWW_MM = calcMt((mu1_TLV + mu2_TLV), m_met->lv());
		  mZTT_coll = calcMZTauTau_coll(mu1_TLV, mu2_TLV, m_met->lv());
		  mZTT_mmc = calcMZTauTau_mmc(mu1_TLV, mu2_TLV, 1/*muon*/, 1/*muon*/);
		  ...
		  //------------------------------------------------------------------------------------
		  if(mu1->q*mu2->q>0){
		    cutnumber = 21.; fillHistos_MM_SRSS1(mcid, cutnumber, weight_ALL_MM); //SS cut: for MM applied only on SS events.
		    if(muEtConeCorr(mu1, m_baseElectrons, m_baseMuons, nt.evt()->nVtx, nt.evt()->isMC)/mu1->pt < 0.1 && muEtConeCorr(mu2, m_baseElectrons, m_baseMuons, nt.evt()->nVtx, nt.evt()->isMC)/mu2->pt < 0.1){
		      cutnumber = 22.; fillHistos_MM_SRSS1(mcid, cutnumber, weight_ALL_MM);
		      cutnumber = 23.; fillHistos_MM_SRSS1(mcid, cutnumber, weight_ALL_MM);
		      if(numberOfFJets(m_signalJets2Lep) == 0){
			weight_ALL_MM *= getBTagWeight(nt.evt());
			cutnumber = 24.; fillHistos_MM_SRSS1(mcid, cutnumber, weight_ALL_MM);
			if(numberOfCBJets(m_signalJets2Lep) == 0){
			  cutnumber = 25.; fillHistos_MM_SRSS1(mcid, cutnumber, weight_ALL_MM);
			  if(nSignalJets >=1){
			    cutnumber = 26.; fillHistos_MM_SRSS1(mcid, cutnumber, weight_ALL_MM);
			    if(m_signalLeptons.at(0)->Pt() >= 30.){
			    cutnumber = 27.; fillHistos_MM_SRSS1(mcid, cutnumber, weight_ALL_MM);
			    cutnumber = 28.; fillHistos_MM_SRSS1(mcid, cutnumber, weight_ALL_MM); //ZVeto
			    
			    //SRSS1
			    if(mtWW_MM > 100.){ //100, 150, 200
			      cutnumber = 29.; fillHistos_MM_SRSS1(mcid, cutnumber, weight_ALL_MM);
			      
			      if(HT_MM >= 200.){
				cutnumber = 30.; fillHistos_MM_SRSS1(mcid, cutnumber, weight_ALL_MM);
			      }
			    }
			    //SRSS2
			    if(mtWW_MM > 150.){ //100, 150, 200
			      cutnumber = 32.; fillHistos_MM_SRSS2(mcid, cutnumber, weight_ALL_MM);
			      if(HT_MM >= 200.){
				cutnumber = 33.; fillHistos_MM_SRSS2(mcid, cutnumber, weight_ALL_MM);
			      }
			    }
			    //SRSS3
			    if(mtWW_MM > 200.){ //100, 150, 200
			      cutnumber = 35.; fillHistos_MM_SRSS3(mcid, cutnumber, weight_ALL_MM);
			      if(HT_MM >= 200.){
				cutnumber = 36.; fillHistos_MM_SRSS3(mcid, cutnumber, weight_ALL_MM);
			      }
			    }
			    //SRSS4
			    if(mtWW_MM > 200.){ //100, 150, 200
			      cutnumber = 38.; fillHistos_MM_SRSS4(mcid, cutnumber, weight_ALL_MM);
			      if(HT_MM >= 200.){
				cutnumber = 39.; fillHistos_MM_SRSS4(mcid, cutnumber, weight_ALL_MM);
			      }
			    }
			  }
			}
		  ...
		  //------------------------------------------------------------------------------------
		  if((mu1->q * mu2->q)<0){
		    cutnumber = 50.; fillHistos_MM_SROS1(mcid, cutnumber, weight_ALL_MM);
		    if(muEtConeCorr(mu1, m_baseElectrons, m_baseMuons, nt.evt()->nVtx, nt.evt()->isMC)/mu1->pt < 0.1 && muEtConeCorr(mu2, m_baseElectrons, m_baseMuons, nt.evt()->nVtx, nt.evt()->isMC)/mu2->pt < 0.1){
		      cutnumber = 51.; fillHistos_MM_SROS1(mcid, cutnumber, weight_ALL_MM);
		      if(numberOfFJets(m_signalJets2Lep) == 0){
			weight_ALL_MM *= getBTagWeight(nt.evt());
			cutnumber = 52.; fillHistos_MM_SROS1(mcid, cutnumber, weight_ALL_MM);
			if(numberOfCBJets(m_signalJets2Lep) == 0){
			  cutnumber = 53.; fillHistos_MM_SROS1(mcid, cutnumber, weight_ALL_MM);
			  if(nSignalJets >=2){  
			    cutnumber = 54.; fillHistos_MM_SROS1(mcid, cutnumber, weight_ALL_MM);
			    TLorentzVector signalJet0_TLV, signalJet1_TLV;
			    signalJet0_TLV.SetPtEtaPhiE(jet0->pt, jet0->eta, jet0->phi, jet0->pt*cosh(jet0->eta));
			    signalJet1_TLV.SetPtEtaPhiE(jet1->pt, jet1->eta, jet1->phi, jet1->pt*cosh(jet1->eta));
			    mjj = Mll(m_signalJets2Lep.at(0), m_signalJets2Lep.at(1));
			    if(mjj >= 50. && mjj <= 100.){
			      cutnumber = 55.; fillHistos_MM_SROS1(mcid, cutnumber, weight_ALL_MM);
			      if(mu1_TLV.Pt() >= 30 && mu2_TLV.Pt() >= 30){
				cutnumber = 56.; fillHistos_MM_SROS1(mcid, cutnumber, weight_ALL_MM);
				float DeltaRmm = fabs(mu1_TLV.DeltaR(mu2_TLV));
				if(DeltaRmm<1.5){
				  cutnumber = 57.; fillHistos_MM_SROS1(mcid, cutnumber, weight_ALL_MM);
				  float DeltaPhiMETmm = fabs((mu1_TLV + mu2_TLV).DeltaPhi(m_met->lv()));
				  if(DeltaPhiMETmm>1.5){
				    cutnumber = 58.; fillHistos_MM_SROS1(mcid, cutnumber, weight_ALL_MM);
				    float mTmmin = (Mt(mu1, m_met) > Mt(mu2, m_met)) ? Mt(mu2, m_met) : Mt(mu1, m_met);
				    if(mTmmin >= 60.){			  
				      cutnumber = 59.; fillHistos_MM_SROS1(mcid, cutnumber, weight_ALL_MM);
				      if(m_met->lv().Pt() >= 80.){
					cutnumber = 60.; fillHistos_MM_SROS1(mcid, cutnumber, weight_ALL_MM);
				      }
				    }
				  }
				}
  ...
  if((m_baseElectrons.size()==1 && m_baseMuons.size() ==1)){
    cutnumber = 15.; fillHistos_EM(cutnumber, weight_ALL_EM); //pass category
    if(m_signalMuons.size()==1 && m_signalElectrons.size()==1 ){
      Muon* mu;
      mu = m_signalMuons.at(0);
      if(fabs(mu->Eta())<=2.4){
	cutnumber = 16.; fillHistos_EM(cutnumber, weight_ALL_EM); //pass nlep
	Electron* el;
	...
	if(m_signalTaus.size() == 0){
	  cutnumber = 17.; fillHistos_EM(cutnumber, weight_ALL_EM);
	  if(m_trigObjWithoutRU->passDilEvtTrig(leptons, 0., nt.evt())){ //valid pT region)   
	    cutnumber = 18.; fillHistos_EM(cutnumber, weight_ALL_EM);

	    if(nt.evt()->isMC || (!nt.evt()->isMC && m_trigObjWithoutRU->passDilTrigMatch(leptons, 0., nt.evt()))){ //match to trigger
	      cutnumber = 19.; fillHistos_EM(cutnumber, weight_ALL_EM);
	      if(!nt.evt()->isMC || CheckRealLeptons(electrons, muons)){
		cutnumber = 20.; fillHistos_EM(cutnumber, weight_ALL_EM);
		...
		float mtWW_EM = calcMt((mu_TLV + el_SS_TLV), m_met->lv());
		float HT_EM = calcHT(el_SS_TLV, mu_TLV, met_SS_TLV, m_signalJets2Lep);
		mZTT_coll = calcMZTauTau_coll(mu_TLV, el_SS_TLV, m_met->lv());
		mZTT_mmc = calcMZTauTau_mmc(mu_TLV, el_SS_TLV, 1/*muon*/, 0/*electron*/);
		...
		cutnumber = 21.; fillHistos_EM_SRSS1(mcid, cutnumber, weight_ALL_EM); //SS cut: applied only on weighted events
		if(muEtConeCorr(mu, m_baseElectrons, m_baseMuons, nt.evt()->nVtx, nt.evt()->isMC)/mu->pt < 0.1){
		  cutnumber = 22.; fillHistos_EM_SRSS1(mcid, cutnumber, weight_ALL_EM);
		  if(fabs(el->d0Sig(true))<=3.0){//|d0/sd0|<3   (for electron)
		    cutnumber = 23.; fillHistos_EM_SRSS1(mcid, cutnumber, weight_ALL_EM);
		    if(numberOfFJets(m_signalJets2Lep) == 0){
		      weight_ALL_SS_EM *= getBTagWeight(nt.evt());
		      cutnumber = 24.; fillHistos_EM_SRSS1(mcid, cutnumber, weight_ALL_EM);
		      if(numberOfCBJets(m_signalJets2Lep) == 0){
			cutnumber = 25.; fillHistos_EM_SRSS1(mcid, cutnumber, weight_ALL_EM);
			if(nSignalJets >=1){
			  cutnumber = 26.; fillHistos_EM_SRSS1(mcid, cutnumber, weight_ALL_EM);   
			  if(el_SS_TLV.Pt()>=20. && mu_TLV.Pt()>=20. && ((el_SS_TLV.Pt()>mu_TLV.Pt() && el_SS_TLV.Pt() >= 30.) || (el_SS_TLV.Pt()<mu_TLV.Pt() && mu_TLV.Pt() >= 30.))){
			    cutnumber = 27.; fillHistos_EM_SRSS1(mcid, cutnumber, weight_ALL_EM);
			    cutnumber = 28.; fillHistos_EM_SRSS1(mcid, cutnumber, weight_ALL_EM); //ZVeto
			    
			    //SRSS1
			    if(mtWW_EM > 140.){ //140, 140, 140
			      cutnumber = 29.; fillHistos_EM_SRSS1(mcid, cutnumber, weight_ALL_EM);
			      
			      if(HT_EM > 200.){
				cutnumber = 30.; fillHistos_EM_SRSS1(mcid, cutnumber, weight_ALL_EM);
				if(METrel>50.){
				  cutnumber = 31.; fillHistos_EM_SRSS1(mcid, cutnumber, weight_ALL_EM);
				}
			      }
			    }
			  }
		...
		//------------------------------------------------------------------------------------
		if((el->q * mu->q)<0){
		  cutnumber = 50.; fillHistos_EM_SROS1(mcid, cutnumber, weight_ALL_EM);
		  if(muEtConeCorr(mu, m_baseElectrons, m_baseMuons, nt.evt()->nVtx, nt.evt()->isMC)/mu->pt < 0.1/* && fabs(el->d0Sig(true))<=3.0*/){//|d0/sd0|<3   (for electron)
		    cutnumber = 51.; fillHistos_EM_SROS1(mcid, cutnumber, weight_ALL_EM);
		    if(numberOfFJets(m_signalJets2Lep) == 0){
		      weight_ALL_EM *= getBTagWeight(nt.evt());
		      cutnumber = 52.; fillHistos_EM_SROS1(mcid, cutnumber, weight_ALL_EM);
		      if(numberOfCBJets(m_signalJets2Lep) == 0){
			cutnumber = 53.; fillHistos_EM_SROS1(mcid, cutnumber, weight_ALL_EM);
			if(nSignalJets >=2){
			  cutnumber = 54.; fillHistos_EM_SROS1(mcid, cutnumber, weight_ALL_EM);
			  mjj = Mll(m_signalJets2Lep.at(0), m_signalJets2Lep.at(1));
			  if(mjj > 50. && mjj < 100.){
			    cutnumber = 55.; fillHistos_EM_SROS1(mcid, cutnumber, weight_ALL_EM);
			    if(el_TLV.Pt()>=30. && mu_TLV.Pt()>=30. && ((el_TLV.Pt()>mu_TLV.Pt() && el_TLV.Pt() >= 30.) || (el_TLV.Pt()<mu_TLV.Pt() && mu_TLV.Pt() >= 30.))){
			      cutnumber = 56.; fillHistos_EM_SROS1(mcid, cutnumber, weight_ALL_EM);
			      float DeltaRem = fabs(el_TLV.DeltaR(mu_TLV));
			      if(DeltaRem<1.5){
				cutnumber = 57.; fillHistos_EM_SROS1(mcid, cutnumber, weight_ALL_EM);
				float mTemmin = (Mt(el, m_met) > Mt(mu, m_met)) ? Mt(mu, m_met) : Mt(el, m_met);
				if(mTemmin > 60.){
				cutnumber = 58.; fillHistos_EM_SROS1(mcid, cutnumber, weight_ALL_EM);
				float DeltaPhiMETem = fabs((el_TLV + mu_TLV).DeltaPhi(m_met->lv()));
				if(DeltaPhiMETem>1.5){
				  cutnumber = 59.; fillHistos_EM_SROS1(mcid, cutnumber, weight_ALL_EM);
				  if(m_met->lv().Pt() >= 80.){
				    cutnumber = 60.; fillHistos_EM_SROS1(mcid, cutnumber, weight_ALL_EM);	      
				    }
				  }
				}
			      }
